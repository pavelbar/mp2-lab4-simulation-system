...
int main()
{
    ...
    while (!_kbhit())
    {
        if (!key) key_on = 1;
        r_all_tact++;//такт
        Task task(rand() % MaxSize + 1);//создание задачи для выполнение которой необходимо 0 - MaxSize тактов 
    ...
    cout << "среднее время выполнения задачи: " << r_task_in_queue << " квантов времени" << endl;
